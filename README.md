...
<br>
  ...
  - **개요**  
  일정 거리 내에 감지된 몬스터를 **지속적으로 추적**하며 방향을 갱신해 이동하는 **발사체**입니다.  

- **핵심 로직**
  - 생성 시 감지된 타겟(몬스터)을 기억하고 추적 대상으로 삼습니다.
  - 매 프레임, **타겟의 현재 위치**를 기준으로 방향 벡터를 계산합니다.
  - 발사체의 회전값을 해당 방향으로 설정하여 **지속적인 회전**이 일어나게 합니다.
  - ProjectileMovementComponent의 Velocity를 갱신해, **정확히 타겟을 향해 이동**합니다.

- **설계 장점**
  - 단순 직선 궤도가 아닌, **목표를 따라 움직이는 유도 방식**으로 스킬 사용의 전략성을 높였습니다.
  - **ProjectileMovementComponent**를 활용한 구현으로 **엔진 친화적이며 최적화된 구조**입니다.
<br>
